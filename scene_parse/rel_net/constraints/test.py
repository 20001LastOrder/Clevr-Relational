...
def test_anti_symmetry():
    ...
    constraints = [get_dag_constraint(), get_anti_symmetry_constraint()]
    criteria = lambda _: constraints[0].cal_loss(_) + constraints[1].cal_loss(_)
    ...
    for i in range(1000):
        ...
        loss = criteria(adj) / torch.numel(adj)
        loss.backward()
    ...
    adj = build_adjacency_matrix(network(x).T, node_size)
    print(adj)
    result = adj
    cumulator = adj
